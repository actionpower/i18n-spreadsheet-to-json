...
describe("i18nFromSpreedSheet.I18nSpreadsheetConverter", () => {
    ...
    describe("generateI18nFiles", () => {
        test("should process sheets specified in config.customIncludedSheets when no fileName is provided", async () => {
            ...
            const converter = new i18nFromSpreedSheet.I18nSpreadsheetConverter(configWithCustomIncludedSheets);
            await converter.generateI18nFiles();
            expect(fetch).toHaveBeenCalledTimes(2);
            expect(fs.writeFileSync).toHaveBeenCalledTimes(4);
            const writeFileCalls = fs.writeFileSync.mock.calls;
            const sheet1Files = writeFileCalls.filter((call) => call[0].includes("Sheet1"));
            const sheet3Files = writeFileCalls.filter((call) => call[0].includes("Sheet3"));
            expect(sheet1Files.length).toBe(2);
            expect(sheet3Files.length).toBe(2);
            expect(Logger.success).toHaveBeenCalledWith("Updated 2 sheets: Sheet1, Sheet3");
        });
        ...
        test("should handle keys with NON_VALUE constant", async () => {
            ...
            await converter.generateI18nFiles();
            expect(fs.writeFileSync).toHaveBeenCalledTimes(4);
            const allWriteCalls = fs.writeFileSync.mock.calls;
            const skipMeKeyFound = allWriteCalls.some((call) => {
                const fileContent = call[1];
                return (typeof fileContent === "string" && fileContent.includes("skipme.key"));
            });
